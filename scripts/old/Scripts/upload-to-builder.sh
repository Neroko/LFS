...
build_version="0.0.1.002"
	...
	startup_check() {															# Startup Setup
		...
		if [ "$command_arg" == "help" ] || [ "$command_arg" == "--help" ] || [ "$command_arg" == "-h" ]; then	# Display help menu and exit
			printf "%s\n" "$top_title"
...
#		menu_line "machine_line" "$(echo -e "${BOLD}-- Info --${STD}")"
		...
		menu_line "option" "V) Virtual Machine Setup"
