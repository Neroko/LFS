...
# The Linux API Headers (in linux-5.13.12.tar.xz) expose the kernel's API
# for use by Glibc.
...
# (API) for the system's C library (Glibc in LFS) to use. This is done by way
# of sanitizing various C header files that are shipped in the Linux kernel
# source tarball.
...
# it requires rsync, which may not be available. The headers are first placed
