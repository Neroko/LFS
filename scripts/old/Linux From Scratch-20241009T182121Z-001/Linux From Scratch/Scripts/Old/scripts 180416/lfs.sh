...
function versionCheck() {
  clear
	source $scriptsFolder$versionCheckFile
...
function newPartition() {
  echo "Test"
  clear
  source $scriptsFolder$newPartitionFile
}
# Main Menu
function mainMenu() {
  function menu() {
...
# Preparing Build Menu
function chapter2() {
  function chapter2Menu() {
    titleHeader
    tput cup 3 "$cc1"; echo "Preparing Build"
    tput cup 4 "$c1"; printf "1: (2.2) Host System Requirements Check";  tput cup 4 "$c2"; echo "$errorsFound"
